...
MODEL_PATHS = {
    "PortScan": MODEL_DIR / "portscan_model.joblib",
    "DDoS": MODEL_DIR / "ddos_model.joblib",
}
...
def get_optional_value(
    ...
    *names,
    ...
    """
    Return the first non-null field value from a row,
    or None when the input does not provide it.
    """
...
def build_model_alerts(
    ...
    for local_index, prediction in enumerate(
        ...
        destination = get_optional_value(
            row,
            "dst_ip",
            "Dst IP",
            "Destination IP",
